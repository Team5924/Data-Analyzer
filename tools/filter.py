...
# Makes a copy of 'arr' and replaces each row, of the copy, with only QT data
def qt(arr):
    ...
    m_array = np.delete(m_array, 0, 1) # deletes the header column
    return m_array

# Makes a copy of 'arr' and replaces each row, of the copy, with only QL data
def ql(arr):
    m_array = np.zeros(np.shape(arr), object)
    x = 0
    for m in range(len(arr)):
